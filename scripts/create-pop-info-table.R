...
long_time_names=names(table(dat$ID))[as.numeric(table(dat$ID))>30]
...
pop_info$min_time_for_power=NA
  for (j in 1:nrow(pop_info)){
    pop= subset(long_dat,long_dat$ID==pop_info$ID[j])[1:30,]
    ...
    pop_info$autocorrelation[j] = acf(pop$popvalue,plot = F)$acf[2]
