...
(citations). However, there are of course notable exceptions including
the Isle Royale study, Iowa corn study, cod, lynx-hair, and many others.
...
For both scientific and management questions, because sampling is
typically expensive, we do not want to sample for longer than is
neccessary (Gerber, DeMaster, and Kareiva 1999).
...
required to estimate trends in population size over time. Rhodes and
Jonzen (2011) examined the optimal allocation of effort between spatial
and temporal replicates. Using simple populations models, they found
that the allocation of effort depends on environmental variation,
spatial and temporal autocorrelation, and observer error. Rueda-Cediel
et al. (2015) also used a modeling approach, but parameterized it for a
...
various questions. In the main manuscript, we determine the minimum time
series length to determine if a population is incresing or decreasing
according to a significant slope coefficient in simple linear
regression.
...
(\(t_{min}\)). We can also examine how the minimum time required changes
with effect size and variability (\textbf{make graph of this here})
...
and continueing until 1 possible contiguous subsample of length 40. Then
we determined the \(T_{min}\) required to be confident (i.e.~have high
...
\caption{(a) Population size of ANIMAL NAME over time. The red line is
the best fit line from linear regresssion. (b) statistical power for
...
variance, and autocorrelation account for about 70.8 of the explained
...
analysis (Gerrodette 1987). Our simulations support past work\ldots{}
