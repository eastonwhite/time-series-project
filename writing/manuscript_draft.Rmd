...
Naturally, one might ask, how long of a time series is actually necessary? This question has important scientific and management implications. Scientifically, we need to know how long a time series must be in order to rigorously test a specific question. Too short of a time series may lead to wrong conclusions given large natural year-to-year variability. From a management perspective, it is important to understand when a trend in abundance over time is actually meaningful or not. For instance the International Union for the Conservation of Nature (IUCN) suggest...

In determining the number of samples required for a particular experiment X quantities are intricately linked: statistical power, effect size, alpha, sample size [@Legg2006]. The relationship between these quantities depends on the specific statistical test used. Formally, statistical power is the one minus the probability of a type II error, or false negative. In other words, it is a measure of how well you could detect a trend given it is actually significant. Prior to an experiment, one could set appropriate levels of power, $\alpha$, and the effect size to estimate the sample size required for the experiment. This approach, however, is not as simple for a time series as the data points are clearly non-independent. 
...
In this paper, we use both simulations and a database of empirical time series to determine the minimum number of years required to address various questions. To demonstrate these techniques, we address three specific questions. 
